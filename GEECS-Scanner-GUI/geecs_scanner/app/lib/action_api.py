...
def get_new_action(action: str) -> Union[None, dict]:
    ...
    if action == "set":
        ...
        return GetStep(
            action="get", device="", variable="", expected_value=""
        ).model_dump()
    elif action == "wait":
        return WaitStep(
            action="wait", wait=1.0
        ).model_dump()  # Default wait duration (you can choose your own)
    elif action == "execute":
...
def generate_action_description(action: dict[str, list]) -> str:
    """Generate a display string summarising all fields of an action step dict."""
    description = "???"
