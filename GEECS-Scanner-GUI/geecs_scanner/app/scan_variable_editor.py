...
def list_of_modes() -> list[str]:
    ...
    return [
        ...
        "absolute",
        "get-only",
    ]  # TODO Need to better implement the intended `get-only` features


# Below is the text that is displayed when a mouse hovers over the "tips" button
relation_tool_tip = (
    "The numerical result of this expression is what the device variable is set to.\n"
...
class ScanVariableEditor(QDialog):
    ...
    def __init__(
        ...
        if config_folder is None:
            ...
            file_buttons = [
                self.ui.buttonVariableSave,
                self.ui.buttonVariableDelete,
                self.ui.buttonCompositeSave,
                self.ui.buttonCompositeNew,
                self.ui.buttonCompositeCopy,
                self.ui.buttonCompositeDelete,
            ]
        ...
        self.ui.lineCompositeNickname.textChanged.connect(
        ...
        self.ui.listCompositeComponents.itemSelectionChanged.connect(
    ...
    def eventFilter(self, source, event):
        ...
        if (
            ...
            and source == self.ui.lineVariableNickname
        ):
            display_completer_list(
                self,
                location=self.ui.lineVariableNickname,
                completer_list=self.get_scan_variable_list(),
            )
            return True
        if (
            event.type() == QEvent.MouseButtonPress
            and source == self.ui.lineCompositeNickname
        ):
            display_completer_list(
                self,
                location=self.ui.lineCompositeNickname,
        ...
        if (
            event.type() == QEvent.MouseButtonPress
            and source == self.ui.lineVariableDevice
        ):
            display_completer_list(
                self,
                location=self.ui.lineVariableDevice,
                completer_list=list(self.database_dict.keys()),
            )
            return True
        if (
            event.type() == QEvent.MouseButtonPress
            and source == self.ui.lineCompositeDevice
        ):
            display_completer_list(
                self,
                location=self.ui.lineCompositeDevice,
                completer_list=list(self.database_dict.keys()),
            )
            return True
        ...
        if (
            event.type() == QEvent.MouseButtonPress
            and source == self.ui.lineVariableVariable
        ):
            display_completer_variable_list(
                self,
                database_dict=self.database_dict,
        ...
        if (
            event.type() == QEvent.MouseButtonPress
            and source == self.ui.lineCompositeVariable
        ):
            display_completer_variable_list(
                self,
                database_dict=self.database_dict,
        ...
        if (
            event.type() == QEvent.MouseButtonPress
            and source == self.ui.lineCompositeMode
            and self.ui.lineCompositeMode.isEnabled()
        ):
            display_completer_list(
                self, location=self.ui.lineCompositeMode, completer_list=list_of_modes()
            )
            return True
    ...
    def check_variable_nickname(self):
        ...
        if nickname in self.get_scan_variable_list():
            device_variable = self.scan_variable_data["single_scan_devices"][
                nickname
            ].split(":")
    ...
    def save_scan_variable(self):
        ...
        self.scan_variable_data["single_scan_devices"][nickname] = (
            f"{device}:{variable}"
        )
        write_dict_to_yaml_file(
            filename=self.file_variables, dictionary=self.scan_variable_data
        )
    ...
    def delete_scan_variable(self):
        ...
        reply = QMessageBox.question(
            self,
            "Delete Scan Variable",
            f"Delete Scan Variable '{nickname}' from list?",
    ...
    def update_visible_composite_information(self):
        ...
        if not name or name not in self.scan_composite_data["composite_variables"]:
            ...
            for var_dict in composite_var["components"]:
                self.ui.listCompositeComponents.addItem(
                    f"{var_dict['device']}:{var_dict['variable']}"
                )
    ...
    def get_component_index(self, device: str, variable: str) -> Optional[int]:
        ...
        component_list = self.scan_composite_data["composite_variables"][name][
            "components"
        ]
    ...
    def add_composite_component(self):
        ...
        self.scan_composite_data["composite_variables"][name]["components"].append(
            new_component
        )
    ...
    def remove_selected_component(self):
        ...
        if component := self.get_selected_composite_component():
            ...
            del self.scan_composite_data["composite_variables"][name]["components"][
                index
            ]
            self.update_visible_composite_information()
    ...
    def get_selected_composite_component(self) -> Optional[dict]:
        ...
        if selected_variable := self.ui.listCompositeComponents.selectedItems():
            ...
            name = self.ui.lineCompositeNickname.text().strip()
            for device_variable in self.scan_composite_data["composite_variables"][
                name
            ]["components"]:
                if (
                    device_variable["device"] == device
                    and device_variable["variable"] == variable
                ):
    ...
    def update_composite_mode(self):
        ...
        if (
            not variable_name
            or variable_name not in self.scan_composite_data["composite_variables"]
        ):
            return

        self.scan_composite_data["composite_variables"][variable_name]["mode"] = (
            self.ui.lineCompositeMode.text()
        )

    def _prompt_new_composite_variable(
        self, message: str, copy_variable: Optional[str] = None
    ):
        ...
        if ok and text:
            ...
            if name in self.scan_composite_data["composite_variables"]:
                logging.warning(
                    f"'{name}' already exists in '{self.file_composite}', cannot create"
                )
                return
            ...
            if copy_variable:
                if copy_variable not in self.scan_composite_data["composite_variables"]:
                    logging.warning(
                        f"'{copy_variable}' does not exist in '{self.file_composite}', cannot create copy"
                    )
                    return
                new_variable = copy.deepcopy(
                    self.scan_composite_data["composite_variables"][copy_variable]
                )
            else:
    ...
    def create_new_composite_variable(self):
        ...
        self._prompt_new_composite_variable(
            message="Enter name for new composite variable:"
        )
    ...
    def copy_composite_variable(self):
        ...
        self._prompt_new_composite_variable(
            message=f"Enter name for copy of '{copy_variable}':",
    ...
    def delete_composite_variable(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            if name in scan_composite_data_actual["composite_variables"]:
                ...
                write_dict_to_yaml_file(
                    filename=self.file_composite, dictionary=scan_composite_data_actual
                )
                logging.info(
                    f"Removed composite variable '{name}' from '{self.file_composite}'"
                )
            else:
                logging.info(
                    f"Removed composite variable '{name}' from unsaved dictionary"
                )
    ...
    def save_composite_variables_file(self):
        ...
        if reply == QMessageBox.Yes:
            write_dict_to_yaml_file(
                filename=self.file_composite, dictionary=self.scan_composite_data
            )
