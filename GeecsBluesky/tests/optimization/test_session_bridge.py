...
from geecs_bluesky.optimization.base_evaluator import BaseEvaluator
...
def _no_real_database(monkeypatch):
    ...
    canonicalizes device names via the module's
    ``_load_experiment_device_names`` seam (gateway ``GeecsDb`` +
    ``GeecsPathsConfig``); off-network that query blocks for the full MySQL
    connect timeout.  Default every test to a seam that raises — the bridge
    must fall back to verbatim requirements.  Canonicalization tests
    install their own fakes on top.
    """
    from geecs_bluesky.optimization import session_bridge

    def _offline_lookup():
        raise RuntimeError("hermetic test: no experiment database")

    monkeypatch.setattr(
        session_bridge, "_load_experiment_device_names", _offline_lookup
    )
...
class TestDeviceRequirementsCanonicalization:
    ...
    def _patch_db(monkeypatch, database: dict) -> dict:
        """Install a fake device-name seam; return call counters."""
        from geecs_bluesky.optimization import session_bridge

        calls = {"constructed": 0}

        def _fake_lookup() -> dict:
            calls["constructed"] += 1
            return {str(name).casefold(): str(name) for name in database}

        monkeypatch.setattr(
            session_bridge, "_load_experiment_device_names", _fake_lookup
        )
        return calls
