...
The stack under test rides the ``optimize`` extra (xopt → torch/botorch,
ScanAnalysis); without it (CI installs GeecsBluesky with no extras) this
whole directory is skipped at collection rather than erroring at import.
"""
...
collect_ignore_glob = (
    ["*"]
    if importlib.util.find_spec("xopt") is None
    or importlib.util.find_spec("scan_analysis") is None
    else []
)
