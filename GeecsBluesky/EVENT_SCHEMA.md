...
| `native_image_save` | the bound plan | Whether the plugin-backed cameras also wrote their LabVIEW per-shot files this run (`false` = their HDF5 stacks are the only record; a camera without a file plugin always wrote its native files). The preset's value, else the experiment default (#738) |
