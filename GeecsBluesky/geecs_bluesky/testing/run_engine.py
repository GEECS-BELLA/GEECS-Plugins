"""RunEngine helpers for local fake-GEECS sandbox scans."""
...
class BackgroundFakeServers(AbstractContextManager["BackgroundFakeServers"]):
    """Run one or more fake GEECS servers in a stoppable background thread."""
    ...
    def __enter__(self) -> "BackgroundFakeServers":
        """Start fake servers and wait until localhost endpoints are ready."""
        self._thread.start()
    ...
    def __exit__(
        ...
        """Stop fake servers and surface background-thread failures."""
        self._stop.set()
        self._thread.join(timeout=self._shutdown_timeout)
...
def disconnect_devices(re: RunEngine, *devices: Device, timeout: float = 10.0) -> None:
    """Best-effort disconnect for devices connected on the RunEngine loop."""
    for device in devices:
        asyncio.run_coroutine_threadsafe(device.disconnect(), re._loop).result(
            timeout=timeout
        )
