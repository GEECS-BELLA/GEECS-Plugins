...
- The move panel's variable picker is an `<input>` over a `<datalist>` rather
  ...
  had. `Move` is now gated on a name the experiment actually has, so a typed
  fragment cannot be submitted.
- New scan opens on the fields every scan type needs — shots per step, trigger
  ...
  changes. This reverses the body ordering half of #896; the preset picker
  stays in the footer.
