...
| `bin_frame(frame, cfg) -> DataFrame` | **improve** (rewrite) | Replace the 145-line stateful `binned_scalars` property with a pure function honoring the same config and output schema, minus the warts: no `id(df)` cache key, no `("count","center")` shape special-case (make count a proper 3-col level or a separate series), label-aligned error assignment, vectorized `mad`, `value_cols` default that excludes `Shotnumber` (as the config docstring already promises; the bin col STAYS in — its per-bin center is the natural X axis, amended during W1c), unit tests for every err mode. `ScanData.binned_scalars` becomes a thin delegate (and `sd.bin(cfg)` finally exists, making the CLAUDE.md true). Freedom granted by finding 1. |
