...
(diffs per camera family, then `Compression=zlib`) and, later, step 5's
tail: `LvNativeFileDataLogic` narrowing to the non-image devices and the
`file_plugin_addr_list` key going.**
