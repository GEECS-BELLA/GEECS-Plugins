...
| phase 2 (`08_gated_batch.md`) | **2a MERGED 2026-09-12 (#843**, GeecsPvaGateway 0.9.0 per-frame scalar attributes, hardware-accepted on Scan009/010 of 26_0911, fleet 0.9.0 ×9); **2b MERGED 2026-09-12 (#850**, GeecsBluesky 0.83.0, GEECS-Schemas 0.22.1: `plans/gated.py`, `devices/sampler.py`, the detector's fly mode, `essential` on `PresetDevice`, the preflight rules, #840's `shot_period`) — **hardware-accepted** on 26_0912: A1–A4/A6–A8 plus a broader device set (Scan003–021), A5 deferred by Sam; **2c built 2026-09-12** (GeecsBluesky 0.84.0, GEECS-Data-Utils 0.31.0: `geecs_data_utils.shot_join`, the s-file from the `shots` rows joined to the stacks, the stack check's gated stamp comparison, the offline re-export) — its acceptance is the re-export of 2b's scans (§5 item 3) |
