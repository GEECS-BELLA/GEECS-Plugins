...
| `POST /api/pause` · `/api/resume` · `/api/stop` · `/api/clear` | the four verbs | `stop` and `pause` carry `{force}`; ownership compares the running item's `user` to the caller's operator name (§4, phase 5) — until then one process identity, `force` a no-op |
