...
| `CaPseudoPositioner` (was `CaPseudoMovable`) | Movable + Locatable + Stageable | a catalog pseudo as an ophyd-async `Transform` under a `DerivedSignalFactory`; `mode: relative` zeroes the components' user offsets at `stage()` and restores at `unstage()` | Replaced by the pseudo arc (`09_pseudo_transform.md`), 2026-09-15 | the namespace registers one per catalog entry |
