...
| `CaPseudoPositioner` (was `CaPseudoMovable`) | Movable + Locatable + Stageable | a catalog pseudo as an ophyd-async `Transform` under a `DerivedSignalFactory`; `mode: relative` zeroes the components' user offsets at `stage()` and restores at `unstage()` | Long-lived: the readback derives from the components' live readbacks, the baselines are per-run by design | **Kept** (rebuilt by the pseudo arc #904, 2026-09-15); the namespace registers one per catalog entry |
