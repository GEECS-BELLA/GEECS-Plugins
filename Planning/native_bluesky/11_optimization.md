...
**Status (2026-09-15): implemented locally on `codex/native-optimization`;
corpus rollout and hardware acceptance remain owed.** Original brief written from a two-part
...
Four PRs (§7), each landed per `/land`. The executing agent reads §2
...
pydantic only, by rule. The legacy pieces: `scan_request.py:371-470`
...
### 4.7 The four keepers in v1 (the corpus regeneration)
...
`plan_names.py` (+`"optimize"` in `NON_SCAN_PLAN_NAMES`),
...
daemon-thread warm import of `xopt` + `torch` guarded by
`importlib.util.find_spec("xopt")`, logged, never fatal (the deleted
...
Each PR per `/land`: branch off `feature/native-bluesky-plans`, scope
