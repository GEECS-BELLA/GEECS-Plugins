...
class CameraImageAnalysis(ScanAnalysis):
    ...
    def load_images_for_bin(self, bin_number: int) -> list[np.ndarray]:
        ...
        for shot_num in shots_in_bin:
            image_file = next(self.path_dict['data_img'].glob(f'*_{int(shot_num):03d}.png'), None)
            if image_file:
                ...
                if self.flag_logging:
                    logging.warning(f"Missing data for shot {int(shot_num)}, adding zero array.")
    ...
    def bin_images(self, flag_save=None):
        ...
        for bin_ind, bin_val in enumerate(unique_bins):
            ...
            binned_data[bin_val] = {'value': self.binned_param_values[bin_ind],
                                    'image': avg_image}
            # print(binned_data[bin_val]["image"].shape)
            # print(bin_val)
            if flag_save:
...
if __name__ == "__main__":
    ...
    tag = ScanData.get_scan_tag(year=2025, month=2, day=13, number=29, experiment_name='Thomson')
    analyzer = CameraImageAnalysis(scan_tag=tag, device_name="HTT-C14_1_ebeamprofile", skip_plt_show=True)
