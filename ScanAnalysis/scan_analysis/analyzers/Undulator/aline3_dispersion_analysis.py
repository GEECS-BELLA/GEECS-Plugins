...
class ALine3DispersionAnalysis(CameraImageAnalyzer):
    ...
    def run_scan_analysis(self):
        ...
        for bin_ind, bin_val in enumerate(unique_bins):
            ...
            shots_in_bin = np.array(self.auxiliary_data[self.auxiliary_data['Bin #'] == bin_val]['Shotnumber'].values)-1
            if len(missing_shots) > 0:
                shots_in_bin = shots_in_bin[~np.isin(shots_in_bin, np.array(missing_shots))]
            ...
            y_projection_norm = y_projection / y_proj_max if y_proj_max != 0 else y_projection
        ...
        axs[0, 0].imshow(
            x_projection_stack, aspect='auto', origin='lower', cmap='viridis',
        ...
        axs[0, 1].imshow(
            y_projection_stack, aspect='auto', origin='lower', cmap='viridis',
