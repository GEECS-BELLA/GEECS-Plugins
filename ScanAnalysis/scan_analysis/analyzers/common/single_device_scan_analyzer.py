...
class SingleDeviceScanAnalyzer(ScanAnalyzer, ABC):
    ...
    def _build_data_file_map(self) -> None:
        """Resolve input references through the shared data-utils mapping rules."""
        self._data_file_map = {}
        try:
            self._data_file_map = map_shot_files(
                self.path_dict["data"],
                self.auxiliary_data,
                device=self.device_name,
                file_tail=self.file_tail,
                prefer_stack=getattr(self, "data_format", None) == "device_hdf5",
                stacks_only=self._reads_stacks_only(),
                file_device=getattr(self, "data_device_name", None),
            )
