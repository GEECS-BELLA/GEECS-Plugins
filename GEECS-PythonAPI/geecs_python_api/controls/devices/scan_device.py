"""Child of GEECS device to support composite variables."""
...
class ScanDevice(GeecsDevice):
    ...
    def __init__(
        self,
        name: str,
        composite_spec_dict: Optional[Dict[str, Any]] = None,
        virtual: bool = False,
    ):
        """
        ...
        if not self.is_composite:
            ...
            for comp in self.components:
                ...
                self.sub_devices[device_name][
                    "instance"
                ].use_alias_in_TCP_subscription = False
    ...
    def _subscribe_to_variables(self) -> None:
        ...
        for device_name, device_data in self.sub_devices.items():
            ...
            logger.info("%s:%s checked %s", sub_device, variables, sub_device.state)
            if not success:
                raise ValueError(
                    f"Failed to subscribe to variables {variables} on device '{device_name}'."
                )
    ...
    def set_reference(self) -> None:
        ...
        if self.mode != "relative":
            raise ValueError(
                "set_reference can only be used for composite variables in relative mode."
            )
        self.reference = {}
        for comp in self.components:
            device_name = comp["device"]
            ...
            logger.info("get %s from %s", device_var, device_name)
            logger.info("object %s state is %s", sub_device, sub_device.state)

            current_value = sub_device.state.get(device_var)
            if current_value is None:
                raise ValueError(
                    f"Failed to retrieve current value for '{device_var}' on '{device_name}'."
                )
    ...
    def set(self, variable: str, value: Any, **kwargs) -> Any:
        ...
        if (
            self.is_composite
            and self.mode in ["relative", "absolute"]
            and variable == "composite_var"
        ):
    ...
    def _calculate_sub_value(self, comp: Dict[str, Any], value: Any) -> Any:
        ...
        try:
            sub_value = ne.evaluate(
                comp["relation"], local_dict={"composite_var": value}
            )
        except Exception as e:
            raise ValueError(
                f"Failed to evaluate relation: {comp['relation']}. Error: {e}"
            )

        return reference_value + sub_value

    def get(
        self, variable: str, use_state: bool = True, **kwargs
    ) -> Union[Dict[str, Any], float, str]:
        """
        ...
        if (
            ...
            try:
                ...
                raise ValueError(
                    f"Failed to evaluate relation: {self.relation}. Error: {e}"
                )
