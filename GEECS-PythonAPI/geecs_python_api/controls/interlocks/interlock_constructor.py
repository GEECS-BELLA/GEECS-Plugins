...
class InterlockConstructor:
    ...
    def __init__(
        ...
        self.device_group = DeviceMonitorGroup(exp_name, staleness_timeout_ms=staleness_timeout_ms)
        self.server: Optional[InterlockServer] = None
        self.monitor_registrations: Dict[str, Callable[[], bool]] = {}
        
        logger.info(f"Initialized InterlockConstructor for experiment '{exp_name}' "
    ...
    def add_monitor(
        ...
        if self.server is not None:
            ...
            logger.debug(f"Queued monitor '{name}' for registration (server not started)")
