...
def cropAndScaleImage(
    ...
    img = img.resize(
        (int(width * scalefactor), int(height * scalefactor)), Image.ANTIALIAS
    )
...
def appendToLog(templateID, documentID, search, servicevar):
    ...
    if checkFileContains(documentID, search, service) == False:
        ...
        print("This Scan is already present in the Log")
        sys.exit()
...
def uploadImage(localimagepath, destinationID):
    ...
    media = MediaFileUpload(localimagepath, mimetype="image/png")
    try:
        file = (
            driveservice.files().create(body=file_metadata, media_body=media).execute()
        )
        if "error" in file:
            # The API executed, but the script returned an error.
