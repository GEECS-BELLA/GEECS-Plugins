...
- **Initial package: the PVA peer of GeecsCAGateway, serving GEECS camera
  ...
  - `GeecsPvaGateway` — one process, N cameras: per-camera gated + supervised
    GEECS TCP subscription (start on first PVA client / stop on last;
    reconnect with 0.5→30 s backoff on socket drops), IMAQ decode off the
    event loop, latest-wins frame slots, GEECS timestamp ladder
    (`acq_timestamp` → `systimestamp`, LabVIEW→Unix), and per-instance
