...
class QWLSIAnalysisTestCase(unittest.TestCase):
    ...
    def generate_test_interferogram(self):
        ...
        wavefront = wavefront_ampl * np.exp(
            -np.square(X - x0) / (2 * x_sig**2) - np.square(Y - y0) / (2 * y_sig**2)
        )
        ...
        interferogram = sum(
            [
                np.cos(
                    ...
                    - 2
                    * np.pi
                    * self.qia.GRATING_CAMERA_DISTANCE
                    * (dsc.nu_x * grad_wavefront.x + dsc.nu_y * grad_wavefront.y)
                )
    ...
    def assertMaxWavefrontMinusBackgroundWithinRange(self):
        self.assertAlmostEqual(
            np.mean(
                self.reconstructed_wavefront[
                    self.distance_from_center_squared < Q_(20.0, "um") ** 2
                ]
            )
    ...
    def test_calculate_wavefront(self):
        self.reconstructed_wavefront = self.qia.calculate_wavefront(
            self.generate_test_interferogram()
        )
