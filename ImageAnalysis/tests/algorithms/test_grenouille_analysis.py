...
from image_analysis.algorithms.grenouille import GrenouilleRetrieval, create_test_grenouille_trace
...
class GrenouilleAnalysisTestCase(unittest.TestCase):
    ...
    def test_full_calculate_pulse(self):
        ...
        E_recovered_at_E_t = np.interp(pulse_t,  gr.E_t, E_recovered, left=0.0, right=0.0)
        self.assertLess(np.max(np.abs(E_recovered_at_E_t - pulse_E)), 0.05)
