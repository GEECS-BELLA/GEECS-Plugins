...
def print_time(label, start_time, do_print=False):
    """
    Prints a string, followed by the change in time from the reference to present, then returns the current time
    ...
    label: str
        String to print
    start_time: float
        The old, reference time
    do_print: bool
        Flag that sets if message gets printed.  Default is False.
...
def analyze_image(input_image, input_params, do_print=False):
    """
    Complete analyzer function used by a MagSpec Analyzer Class to analyze a single raw image.  Requires a dictionary
    of input parameters assembled by the MagSpec Analyzer Class, so it is recommended to only call this function through
    the analyze_image function of these Analyzer Classes.

    Parameters
    ----------
    input_image:
        A raw image from either a LabView input or from reading a file. Formatted as a 2D array of np.float32
    input_params: dict
        Python dict that defines all the input parameters.  This is assembled by the MagSpec Analyzer Class
    do_print: bool
        Optional flag set to False by default.  Set to True to print evaluation time receipts.

    Returns
    -------
    tuple
        A Tuple of the processed image, a dictionary of scalar outputs, and lineouts stacked in a 2D array.  The dictionary
        has labels for the scalar outputs, while the lineouts currently supported are the energy axis and the charge density
        projection array.
    """
    ...
    normalization_factor = input_params["charge_normalization_factor_pC/count"]
    ...
    calibration_factor = input_params["transverse_calibration_factor_um/pixel"]
    ...
    if np.sum(charge_arr) == 0:
        ...
        central_energy = input_params["optimization_central_energy_MeV"]
        bandwidth_energy = input_params["optimization_bandwidth_energy_MeV"]
        ...
        do_transverse = input_params["do_transverse_calibration_bool"]
        if do_transverse:
            binsize = input_params["transverse_slice_bin_size_pixels"]
            slice_threshold = input_params["transverse_slice_threshold_factor"]
