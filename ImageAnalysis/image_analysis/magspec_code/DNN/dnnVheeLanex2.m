...
scan = [42]; % for auto mode, first scan
bgScan = 2;
%roiRmrad = 0.5; % radius for charge-sum ROI [mm]
%roiXY = [2,-3.8]; % roi location [mrad]
roiRmm = 2; % radius for charge-sum ROI for sample plane [mm]
smpStg = [7,21]; % sample stage for ROI
roiXY(1) = smpStg(1)-3; roiXY(2) = 14-smpStg(2);
%roiXY = [1,-7]; % roi location [mm]
%nc2Gy = 0.97*12.12/0.185; % nC to dose factor (depends on roiRMrad)
cd2Gy = (4.227+4.425)*0.5; % calibration, pc/mm^2/Gy

%mgspcA = 10; % magspec acceptance in radius [mrad]
mgspcA = 17; % magspec acceptance in radius at sample plane [mm]
src2LnxM = 1.500; % source to lanex in m
src2Smpl = 1.580; % sourc to sample z [m]
fntSz = 12; % font size for fig
viewRoi = [-18 18 -18 18]; % roi for figure [mrad]
...
% projection to sample plane
xAxMMs = xAxMM * src2Smpl / src2LnxM; % x axis [mm] in sample plane
yAxMMs = yAxMM * src2Smpl / src2LnxM; % y axis [mm] in sample plane
dmmS = xAxMMs(2)-xAxMMs(1);

% roi (may introduce logic here)
radD = [0:0.01:2*pi];   % radian for drawing
%[xMesh,yMesh] = meshgrid(xAxMrad,yAxMrad);
[xMesh,yMesh] = meshgrid(xAxMMs,yAxMMs); % sample plane
rMesh = sqrt((xMesh-roiXY(1)).^2+(yMesh-roiXY(2)).^2);% r mesh [mm]
rLgc = rMesh<=roiRmm;% ROI logic
...
img = bgImg; acImg = img*0; img2 = img*0;
...
while scanN>0 % scan loop
    ...
    while shotN<=nmbSht % loop goes on till exceeds nmb of shot
        ...
        img2(:,:) = 0.001*acImg/dmmS^2; % accumulated charge density [pC/mm^2]
        meanCD = mean(img2(rLgc)); % accumulated roi-mean charge density [pc/mm^2]
        ...
        info{6} = ['roi-Acc chrg den: ' num2str(meanCD,4) ' pc/mm^2'];

        %% figure
        set(0,'CurrentFigure',fig1)
        subplot(2,3,2)
        pcolor(xAxMMs,yAxMMs,img)
        hold on
        plot([xAxMMs(1) xAxMMs(end)],[0 0],'k--')
        plot([0 0],[yAxMMs(1) yAxMMs(end)],'k--')
        plot(roiRmm*cos(radD)+roiXY(1),roiRmm*sin(radD)+roiXY(2),'w--')
        ...
        doseAR = meanCD/cd2Gy; % ROI-dose [Gy]
        doseTx = ['roi-dose: ' num2str(doseAR,4) ' Gy'];
        dsRate = doseAR/shotN; % dose rate [Gy/shot]
        drTx = ['dose rate: ' num2str(1000*dsRate,4) ' mGy/shot'];

        %pcolor(xAxMMs,yAxMMs,acImg)
        pcolor(xAxMMs,yAxMMs,img2)
        hold on
        plot([xAxMMs(1) xAxMMs(end)],[0 0],'k--')
        plot([0 0],[yAxMMs(1) yAxMMs(end)],'k--')
        plot(roiRmm*cos(radD)+roiXY(1),roiRmm*sin(radD)+roiXY(2),'k--')
        %plot(mgspcA*cos(radD),mgspcA*sin(radD),'w--')
        %plot(spotA(1),spotA(2),'rx','MarkerSize', 10)
        text(-11,8,doseTx,'FontSize',fntSz,'Color','w');
        text(-11,6,drTx,'FontSize',fntSz,'Color','w');
        ...
        colorbar
        axis([-12 12 -15 10])
        xlabel('mm'); ylabel('mm');title('acc charge den [pc/mm^2]')
        aa = axis;
        set(gca,'fontsize',fntSz);
    ...
    %% accumulated image
    spotA = fSpotAnalysisV02(xAxMMs,yAxMMs,acImg,1);
    ...
    infoA{5} = ['xPos: ' num2str(spotA(1),3) ' mm'];
    infoA{6} = ['yPos: ' num2str(spotA(2),3) ' mm'];
    infoA{7} = ['sample x = ' num2str(spotA(1)+3,3) ' mm'];
    infoA{8} = ['sample y = ' num2str(-spotA(2)+14,3) ' mm'];
    ...
    fPltInfV02(subplot(1,3,1),infoA,8,fntSz);
    ...
    if addScan==1
        ...
        ictC = str2double(logDat1(:,fLogClmnFindV01(logTtl1,'HTT-VHEE_ICT_Picoscope_charge_pC'))); % ict charge
