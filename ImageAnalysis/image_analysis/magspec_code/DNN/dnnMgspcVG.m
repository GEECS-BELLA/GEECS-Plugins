...
fig2 = figure(2);
set(gcf,'color',[1 1 1]);
set(fig2, 'Position', [100 30 1000 800]);
fig3 = figure(3);
set(gcf,'color',[1 1 1]);
set(fig3, 'Position', [100 30 350 350]);

%% input parameters
day = '26_0303';    % day string to locate data to analyze
mode = 1;   % mode (1: server, 2: local)
%scan = input('inp2ut scan number like [1:4,8]: ');
scan = [18:54];%[11,12,14, 18, 62, 64, 68, 69, 70, 160, 161, 162, 163, 165, 166, 168, 169, 170, 171, 185];%[14 16 17 24:27];%[5,8:10,15,16,25];
roiM = [5,200];   % roi momentum [MeV]
roiA = [-10,10];   % roi angle [mrad]
addScan = 1;        % add summary to s-file
figV = 'on';   % off for fast process, on to check, figure visibility
...
for scanN=scan    % scan loop
    ...
    aa = axis;
    axis([roiM(1) roiM(2)*1.08 0 aa(4)])
    cc = caxis;
    set(gca,'fontsize',fntSz);
...
%%
    ...
    namOut = [anlNDir,filesep,'Scan',scanNS,'_average.txt'];
    ttlB{1} = ttlE{1};
    ttlB{2} = ttlE{3};
    ttlB{3} = 'stddev';
    ok = fTxtOutV01(namOut,3,ttlB,[x.mmt,mean(wtrFllM)',std(wtrFllM)']);
