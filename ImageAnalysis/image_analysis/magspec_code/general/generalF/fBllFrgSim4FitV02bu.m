function [arry,pulse] = fBllFrgSim4FitV02bu(cmpLPeak,dsprsnIn,cmpPara,cmrPara,axesP,diag,figH,varargin)
...
for jj=1:nmbTry
    ...
    %cmrThre = dsprsnIn(jj,6);  % camera threshold count
    %cmrSig = dsprsnIn(jj,7);    % camera sigma
    %dsprsnIn(jj,6:7) = 0; % 0 dispersions for 0th and 1st
    ...
    IsigP(:,:) = (cmrThre+peakCnt)*IsigP/max(max(IsigP));  % normalized to ccd count+thre
    ...
    IsigPB(:,:) = IsigPB*peakCnt/max(max(IsigPB));  % re-normalized to peak
