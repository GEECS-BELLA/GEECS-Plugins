%%%%%%%%%% Function fWvFrmPrcAllV01.m %%%%%%%%%%
% Wave Form Process All ver.1
% Kei Nakamura
...
function out = fWvFrmPrcAllV01(pthD, tim)   % function header
...
for ii=1:wNum
    ...
    out(sht,1) = sht;
    [out(sht,2) out(sht,3)] = fWvFrmPrcV02(wLst(ii).name,tim);  % signal, charge[pC], S/N
    waitbar(ii/wNum);
end
close(h);
cd(cDir);
