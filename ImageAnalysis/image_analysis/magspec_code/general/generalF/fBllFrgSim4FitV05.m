function [arry,pulse,IsigPB] = fBllFrgSim4FitV05(cmpLPeak,dsprsnIn,cmpPara,cmrPara,diag,figH,eS,osPrp,varargin)
...
% 2015/4/1 ver.4: 800 nm
% 2015/9/29 ver.5: 12th order, de-factorial
...
%% inputs
if nargin>8
    logSclL = varargin{1};  % log scale low for sumamry plot
else
    logSclL = 0;
end
...
%% constants
mSize = osPrp.mSize;       % matrix size for time and freq FFT
tStp = osPrp.dt;       % matrix size for time and freq FFT
[T,W] = fFrogFFTParaV02(mSize,tStp); % time, omega, d omega
fctArry(1,13) = -1; % factorial array init
dspWeight(1,13) = 1;   % log to real conversion weight
dspWeightLF(1,13) = 1;  % log to fit conversion weight
dspWeightRF(1,13) = 1;  % real to fit conversion weight
for ii=1:12
    fctArry(1,13-ii) = -factorial(ii);
    if ii>5
        dspWeight(1,13-ii) = 10^(2*(ii-6));
        dspWeightLF(1,13-ii) = 10^(ii-7);
        dspWeightRF(1,13-ii) = 10^(-ii+5);
    else dspWeight(1,13-ii) = 1;
        dspWeightLF(1,13-ii) = 1;
        dspWeightRF(1,13-ii) = 1;
    end
end
...
tmpTxtDsp = (dsprsnIn(1,:).*fctArry./dspWeightLF.*dspWeight);   % for figure
diagDsp = fBllDiagDsprsnV02(800,diag,dspOrd);
diagDsp = diagDsp./fctArry; % de-factorial
diagDspM = meshgrid(diagDsp,1:nmbTry);    % diagnostic dispersion mesh
weightM = meshgrid(dspWeightRF,1:nmbTry);    % dispersion weight mesh
dsprsnIn = dsprsnIn./weightM; % de-weight
...
cmpDsp = zeros(numel(D),13);
for kk=1:numel(D)
    cmpDsp(kk,:) = fCpaDispersionV03(D(kk),N,800,alpD,-1,dspOrd)./fctArry;  % de-factorial
end
...
for jj=1:nmbTry
    ...
    for ii=1:nmbImg
        ...
        if figH~=0&&nmbImg>1
            ...
            hold on
            text(aa(1),-1,p2txt,'color',[0,0,0]);
            text(aa(1),-2,p3txt,'color',[0,0,0]);
            text(aa(1),-3,p4txt,'color',[0,0,0]);
...
if figH~=0&&nmbImg==1
    ...
    subplot(3,3,7)
    pcolor(cmrPara.T,cmrPara.W,log10(IsigPB))
    shading interp
    aa = caxis;
    caxis([-2,aa(2)])
    colorbar
...
elseif figH~=0&&nmbImg>=1
    ...
    plot([0 szX],[0 0],'w--')
    for pp=1:nmbImg
        plot([szXD*pp-szXD/5 szXD*pp-szXD/5],[cmrPara.W(1) cmrPara.W(end)],'w--')
    end
    hold off
    tx1 = ['p2=' num2str(1e-3*round(tmpTxtDsp(1,11))) ' kfs^2'];
    tx2 = [', p3=' num2str(1e-1*round(1e-2*tmpTxtDsp(1,10))) ' kfs^3'];
    tx3 = [', p4=' num2str(1e-3*round(1e-3*tmpTxtDsp(1,9))) ' Mfs^4'];
    tx4 = [', p5=' num2str(1e-2*round(1e-4*tmpTxtDsp(1,8))) ' Mfs^5'];
    tx5 = [', p6=' num2str(round(1e-6*tmpTxtDsp(1,7))) ' Mfs^6'];
    tx6 = [', p7=' num2str(round(1e-9*tmpTxtDsp(1,6))) ' Gfs^7'];
    tx7 = [', p8=' num2str(round(1e-12*tmpTxtDsp(1,5))) ' Tfs^8'];
    tx8 = [', p9=' num2str(round(1e-12*tmpTxtDsp(1,4))) ' Tfs^9'];
    tx9 = [', p10=' num2str(round(1e-15*tmpTxtDsp(1,3))) ' Pfs^{10}'];
    title([tx1 tx2 tx3 tx4 tx5 tx6 tx7 tx8 tx9])

    % log scale
    subplot(6,nmbImg,1+nmbImg*3:4*nmbImg)
    pcolor([1:szX],cmrPara.W,log10(IsigPB))
    shading interp
    colormap jet
    cax = caxis;
    caxis([logSclL cax(2)])
