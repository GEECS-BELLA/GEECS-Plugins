...
if ~isempty(popu)
    ...
    switch tfct
        ...
        case 7  % enge function 1st
            longfct = 1./(1+exp(longV(1,:)+longV(2,:).*longX/argf));
        case 8  % enge function 2nd
            dlt = longV(1,:);
            dlt = dlt + longV(2,:).*longX/argf;
            dlt = dlt + longV(3,:).*(longX/argf).^2;
            longfct = 1./(1+exp(dlt));
        case 9  % enge function 3rd
            dlt = longV(1,:);
            dlt = dlt + longV(2,:).*longX/argf;
            dlt = dlt + longV(3,:).*(longX/argf).^2;
            dlt = dlt + longV(4,:).*(longX/argf).^3;
            longfct = 1./(1+exp(dlt));
        case 10  % enge function 4th
            dlt = longV(1,:);
            dlt = dlt + longV(2,:).*longX/argf;
            dlt = dlt + longV(3,:).*(longX/argf).^2;
            dlt = dlt + longV(4,:).*(longX/argf).^3;
            dlt = dlt + longV(5,:).*(longX/argf).^4;
            longfct = 1./(1+exp(dlt));
        case 11  % enge function 5th
            dlt = longV(1,:);
            dlt = dlt + longV(2,:).*longX/argf;
            dlt = dlt + longV(3,:).*(longX/argf).^2;
            dlt = dlt + longV(4,:).*(longX/argf).^3;
            dlt = dlt + longV(5,:).*(longX/argf).^4;
            dlt = dlt + longV(6,:).*(longX/argf).^5;
            longfct = 1./(1+exp(dlt));
