...
class HiResMagCamAnalyzer(BeamAnalyzer):
    ...
    def analyze_image(
        ...
        """Run preprocessing, evaluate bowtie fit, and return ImageAnalyzerResult.
        ...
        # Create result with bowtie fit data
...
if __name__ == "__main__":
    ...
    if file_path.exists():
        results: ImageAnalyzerResult = image_analyzer.analyze_image_file(
            image_filepath=file_path
        )
        image_analyzer.visualize(results)  # shows exactly one figure
        print(f"Emittance proxy: {results.scalars['emittance_proxy']:.2f}")
        print(f"Total counts: {results.scalars['total_counts']:.2f}")
    else:
