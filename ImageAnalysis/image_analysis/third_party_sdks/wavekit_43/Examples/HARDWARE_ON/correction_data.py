...
hasoengine = wkpy.HasoEngine(config_file_path = wfs_config_file_path)
...
corr_data_manager.set_calibration_prefs(0.2)
...
for i in range(specs.nb_actuators):
    ...
    if(prefs.validity==wkpy.E_ACTUATOR_CONDITIONS.VALID):
        ...
        hasoslopes_array[(2 * i)] = hasoengine.compute_slopes(image, False)[1] #Get only HasoSlopes
        """Get pull interaction slopes
        """
        wavefrontcorrector.move_to_absolute_positions(pull)
        image = camera.get_raw_image()
        hasoslopes_array[(2 * i) + 1] = hasoengine.compute_slopes(image, False)[1] #Get only HasoSlopes
...
corr_data_manager.save_backup_file('./../OUT_FILES/correction_data_backup.aoc', 'Exemple Test')
print('Correction data saved to file correction_data_backup.aoc in OUT_FILES directory.')
