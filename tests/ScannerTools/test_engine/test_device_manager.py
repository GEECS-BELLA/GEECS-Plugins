...
from geecs_scanner.engine import DeviceManager
...
def test_device_manager_functionality():
    ...
    composite_variable_path = (
        config_root_path / "scan_devices" / "composite_variables.yaml"
    )
    ...
    device_manager.reinitialize(
        config_dictionary=config
    )  # Need to first load it, otherwise will try the 'live' folder
    ...
    test_observable = [
        f"{TEST_DEVICE_NAME}:{TEST_VARIABLE_NAME_1}",
    ...
    assert (
        f"{TEST_SCAN_DEVICE}:{TEST_SCAN_VARIABLE}" in device_manager.async_observables
    )
